...
```{r}
fig <- plot_ly(df, x = ~Brand, y = ~FastCharge_KmH, type = 'scatter')
...
### highest price
```{r}
fig <- plot_ly(df, x = ~Brand, y = ~PriceEuro, type = 'scatter', mode = 'markers')
...
### highest speed
```{r}
fig <- plot_ly(df, x = ~Brand, y = ~TopSpeed_KmH, color = ~TopSpeed_KmH, size = ~TopSpeed_KmH)
